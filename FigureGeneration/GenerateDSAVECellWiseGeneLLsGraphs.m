...
%% B Cells
%{
progbar = ProgrBar('Cell-wise: Fig F');

tic
[llsLcb, genesLcb, genellsLcb] = DSAVEGetSingleCellDivergence(lcbSub, 200, progbar.GetSubContext(1));
progbar.Done();
toc
...
%also need to remove only NaN cells if there are any - that is not the case
%here however
worstVals = nanmin(genellsLcb, [], 2);
nanGenesSel = isnan(worstVals);
noNanGenes = genesLcb(~nanGenesSel);
worstValsNoNan = worstVals(~nanGenesSel);
[~,worstValsIndices] = sort(worstValsNoNan);
worstValsNoNan(worstValsIndices(1:40))
noNanGenes(worstValsIndices(1:40))
%    {'IGLC7' } - Immunoglobin, likely plasma cells, ok
%    {'IGLC3' } - Immunoglobin, likely plasma cells, ok
%    {'IGHA1' } - Immunoglobin, likely plasma cells, ok
%    {'IGLC2' } - Immunoglobin, likely plasma cells, ok
%    {'IGHM'  } - Immunoglobin, likely plasma cells, ok
%    {'IGHG3' } - Immunoglobin, likely plasma cells, ok
%    {'IGKC'  } - Immunoglobin, likely plasma cells, ok
%    {'IGHG1' } - Immunoglobin, likely plasma cells, ok
%    {'MALAT1'} - Unclear
%    {'IGHG4' } - Immunoglobin, likely plasma cells, ok
%    {'IGLL5' } - Immunoglobin, likely plasma cells, ok
%    {'IGHG2' } - Immunoglobin, likely plasma cells, ok
%    {'IGHD'  } - Immunoglobin, likely plasma cells, ok
%    {'TOMM7' } - Mitochondrial membrane protein, unclear
%    {'IGHA2' } - Immunoglobin, likely plasma cells, ok
%    {'JCHAIN'} - Immunoglobin, likely plasma cells, ok
%    {'RPL13'   }
%    {'TMSB10'  }
%    {'MED11' } - Transcription, unclear
%    {'IGHE'    } - Immunoglobin, likely plasma cells, ok
%    {'RPL35A'} - Ribosomal protein, unclear
%    {'COPE'  } - Golgi, unclear
%    {'FTH1'  } - Iron Storage
%    {'IGHGP' } - Immunoglobin, likely plasma cells, ok
%    {'SEC23B'} - Unclear
%    {'FKBP2' }
%    {'TACO1' } - related to cytochrome C, unclear
%    {'RPS3A' }
%    {'MORC3' }
%    {'SFTPC' } - sufactant used in lung, so likely comes from lung-related cells and not B cells
%    {'RPL26' }
%    {'GNAS'  }
%    {'LTA4H' } - enzyme, unclear
%    {'DOPEY2'} - unclear
%    {'MRPL34'}
%    {'HBB'   } - red blood cell precursor
%    {'FTL'   } - Stores iron in cells, high in red blood cell precursors?
%    {'LAT2'    } - One cell is high, could possibly be something else than a B cell
%    {'IGLV9-49'} - Immunoglobin, likely plasma cells, ok
%    {'SHMT2' } - Metabolic enzyme, seems it can be high in B cells.



lcbSubTPM.data(strcmp(lcbSubTPM.genes,'MALAT1'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'HBB'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'TPSB2'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'TPSAB1'),:)
...
lcbSub.data(strcmp(lcbSub.genes,'LAT2'),:)
lcbSub.data(strcmp(lcbSub.genes,'MS4A2'),:)

lcbSubTPM.data(strcmp(lcbSubTPM.genes,'SFTPC'),:)
lcbSub.data(strcmp(lcbSub.genes,'SFTPC'),:)

lcbSubTPM.data(strcmp(lcbSubTPM.genes,'SLC52A2'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'CPTP'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'EAPP'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'CCL4L2'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'NKG7'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'RPS15'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'STK17A'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'PTGDS'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'RSRP1'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'CCL4'),:)
lcbSubTPM.data(strcmp(lcbSubTPM.genes,'FTL'),:)
...
%plot against number of umis
numUMIsLcb = sum(lcbSub.data,1);

%find bad cells:
threshold = 20000;

%red blood cell precursors:
selHBB = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'HBB'),:) > threshold;
selHBA1 = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'HBA1'),:) > threshold; %not among 40 most varying
selHBA2 = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'HBA2'),:) > threshold; %not among 40 most varying
...
selTPSB2 = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'TPSB2'),:) > 5000;%the histogram looks separated here
selTPSB2Low = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'TPSB2'),:) > 0;%presence of the gene
selTPSAB1 = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'TPSAB1'),:) > 5000;%the histogram looks separated here
...
%lung tissue:
selSFTPC = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'SFTPC'),:) > 10000;
selLung = selSFTPC;
%SFTPCExpr = log2(lcbSubTPM.data(strcmp(lcbSubTPM.genes,'SFTPC'),:) + 1);
%figure
%histogram(SFTPCExpr)

TPSB2Expr = log2(lcbSubTPM.data(strcmp(lcbSubTPM.genes,'TPSB2'),:) + 1);
TPSAB1Expr = log2(lcbSubTPM.data(strcmp(lcbSubTPM.genes,'TPSAB1'),:) + 1);
figure
histogram(TPSB2Expr)
%figure
...
%selFTL = lcbSubTPM.data(strcmp(lcbSubTPM.genes,'FTL'),:) > threshold/2;

selMan = [1992 1845 919];
...
logNumUMIsLcb = log2(numUMIsLcb);
figure
gscatter(llsLcb,logNumUMIsLcb,group,col,'o',3);
xlabel('Log likelihood')
ylabel('log2(UMI counts)')
title('Classification of Divergent B Cells');
set(gca,'FontSize',11);
axis([-1150 -300 8.6 16]);

%make sure the colored plots are on top, so draw them again:
hold on;
scatter(llsLcb(selMast),logNumUMIsLcb(selMast),10,[0,0.5,0]);
hold on;
scatter(llsLcb(selRb),logNumUMIsLcb(selRb),10,[1,0,0]);
hold on;
scatter(llsLcb(selLung),logNumUMIsLcb(selLung),10,[0,0,0]);
hold on;
scatter(llsLcb(selMan),logNumUMIsLcb(selMan),10,[0,0,1]);
legend({'No anomaly found', 'Red blood cell precursors', 'Mast cells', 'Lung cells', 'Manually investigated'});

figure
gscatter(llsLcb,logNumUMIsLcb,lcbSub.subCellType,[],'o',3);
xlabel('Log likelihood')
ylabel('log2(UMI counts)')
title('Classification of Divergent B Cells');
set(gca,'FontSize',11);
...
figure
gscatter(llsLcb,logNumUMIsLcb,group2,col,'o',3);
xlabel('Log likelihood')
ylabel('log2(UMI counts)')
title('Classification of Divergent B Cells');
set(gca,'FontSize',11);
axis([-2600 -700 8.6 16]);
